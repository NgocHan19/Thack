...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/send-magiclink', {email})
      if (response.data.success) {
        setMessage('Không thể gửi link, vui lòng thử lại')
      } else {
        setMessage('Link xác thực đã được gửi đến email')
      }
    } catch(error) {
  ...
  return (
    <div className="relative w-full h-auto">
      ...
      {/* Đăng nhập button */}
      <button className="absolute left-[400px] top-[320px] w-[827px] h-[60px] bg-[#3498DB] rounded-[5px] flex items-center justify-center" type='submit'>
      ...
      {message && (
        <div className="absolute left-[400px] top-[450px] text-[#3498DB] text-[20px] font-bold">
          {message}
        </div>
      )}
    </div>
